...
generacion_variable_sintetica = function(df){
  ...
  # Número de filas sobre las que se itera 
  nrows = nrow(df)
...
graficacion_histograma = function(params, coeficiente_original, p_value_original){
  ...
  histograma = ggplot(params, aes(x = coef)) +
    ...
    geom_hline(yintercept = p_value_original, linetype = "dashed", color = "red") +  # Horizontal line at y = 50
    geom_vline(xintercept = coeficiente_original, linetype = "dashed", color = "red")+    # Vertical line at x = 5
...
test_de_randomizacion = function(df_original, formula_estimacion, coef_interes, estimacion_original, sample_size){
  ...
  for (i in 1:sample_size){
    ...
    df_panel = generacion_base_datos_panel(df)
